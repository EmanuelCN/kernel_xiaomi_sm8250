	...
	if (rw_op.len < 0) {
		...
		if (RWREG_OP_READ == rw_op.type) {
			if (rw_op.len == 1) {
				...
				rw_op.res =
					fts_i2c_read(client, &reg, 1,
						     rw_op.opbuf, rw_op.len);
			}
...
static struct attribute *fts_attributes[] = {
	...
	&dev_attr_fts_force_upgrade.attr, &dev_attr_fts_driver_version.attr,
	&dev_attr_fts_hw_reset.attr,	  &dev_attr_fts_irq.attr,
	&dev_attr_fts_fod_test.attr,	  NULL
};
