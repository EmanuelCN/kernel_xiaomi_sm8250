...
int fts_upgrade_bin(struct i2c_client *client, char *fw_name, bool force)
{
	...
	if (ret == 0) {
		fw_file_len = fw->size;
		fw_file_buf =
			(u8 *)kzalloc(fw_file_len * sizeof(u8), GFP_KERNEL);
		if (fw_file_buf == NULL) {
...
static bool fts_fwupg_need_upgrade(struct i2c_client *client)
{
	...
	if (fwvalid) {
		...
		FTS_INFO("fw version in tp:0x%x, host:0x%x", fw_ver_in_tp,
			 fw_ver_in_host);
		...
		if (upgrade_flag) {
			...
			if (upg->func->upgrade) {
				...
				if (ret < 0) {
					...
					FTS_INFO(
						"success upgrade to fw version 0x%02x",
						ver);
					break;
				}
			} else {
				...
				if (fts_param_need_upgrade(client)) {
					...
					if (ret < 0) {
						...
						fts_param_get_ver_in_tp(client,
									&ver);
						FTS_INFO(
							"success upgrade to fw param version 0x%02x",
							ver);
						break;
					}
				} else {
...
void fts_fwupg_auto_upgrade(struct fts_ts_data *ts_data)
{
	...
	if (ret < 0)
		...
		FTS_ERROR(
			"**********tp fw(app/param) upgrade success*********\n");
...
static int fts_get_lockdown_info(struct fts_ts_data *ts_data)
{
	...
	ret = fts_flash_read(ts_data->client,
			     ts_data->pdata->lockdown_info_addr,
			     ts_data->lockdown_info, FTS_LOCKDOWN_INFO_SIZE);
	if (ret < 0) {
	...
	for (i = 0; i < FTS_LOCKDOWN_INFO_SIZE; i++)
		FTS_INFO("Lockdown info[%d] = 0x%02x", i,
