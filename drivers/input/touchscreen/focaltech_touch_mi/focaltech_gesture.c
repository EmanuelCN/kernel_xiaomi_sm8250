...
void fts_gesture_recovery(struct i2c_client *client)
{
	if ((ENABLE == fts_gesture_data.mode) &&
		...
		fts_gesture_reg_write(client, FTS_REG_GESTURE_DOUBLETAP_ON,
				      true);
...
int fts_gesture_resume(struct i2c_client *client)
{
	...
	if (fts_gesture_data.active == DISABLE) {
		FTS_DEBUG(
			"gesture in suspend is failed, no running fts_gesture_resume");
		return -EINVAL;
	}
	ret = fts_gesture_reg_write(client, FTS_REG_GESTURE_DOUBLETAP_ON,
				    false);
	if (ret) {
