...
static const uint32_t aw2015_ton_toff[] = {
	40,   130,  260,  380,	510,  770,  1040, 1600,
...
static void aw2015_brightness_work(struct work_struct *work)
{
	...
	if (led->cdev.brightness > 0) {
		...
		aw2015_write(led, AW2015_REG_ILED1_1 + led->id,
			     led->cdev.brightness);
		aw2015_write(led, AW2015_REG_PWM1 + led->id,
			     AW2015_LED_ON_PWM_MASK);
		aw2015_read(led, AW2015_REG_LEDEN, &val);
...
static void aw2015_led_breath_set(struct aw2015_led *led,
				  unsigned long breathing)
{
	...
	if (breathing > 0) {
		...
		aw2015_write(led, AW2015_REG_PAT1_T3 + led->id * 5,
			     (led->pdata->slot_time_ms << 4 |
			      led->pdata->delay_time_ms));

		reg_patx_t4 |=
			(led->pdata->mpulse & 0x03)
			<< 4; /* BIT[5:4] - multiple pulse mode selection */
		switch (breathing) {
		...
		case 4: /* multi-pattern, multi-color, multi-pulse */
			reg_patx_t4 |= BIT(
				6); /* pattern switch enable, active only in true-color pattern mode */
			reg_patx_t4 |=
				led->pdata->cex &
				0x0f; /* BIT[3:0] - color#x display enable */
		...
		aw2015_write(led, AW2015_REG_PAT1_T4 + led->id * 5,
			     reg_patx_t4);
				     ...
				     const char *buf, size_t len)
{
	...
	rc = sscanf(buf, "%d %d %d %d %d %d", &delay_time_ms, &rise_time_ms,
		    &hold_time_ms, &fall_time_ms, &slot_time_ms, &off_time_ms);
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < 4; ++i) {
		if (led->pdata->cex & BIT(i))
			len += snprintf(tmp_buf + len, buf_len - len, "[%d] ",
					led->pdata->currents[i]);
		else
			len += snprintf(tmp_buf + len, buf_len - len, "%d ",
					led->pdata->currents[i]);
	}
				     ...
				     const char *buf, size_t len)
{
	...
	sscanf(buf, "%d %d %d %d", &led->pdata->currents[0],
	       &led->pdata->currents[1], &led->pdata->currents[2],
				...
				struct device_attribute *attr, char *buf)
{
	...
	uint8_t i = 0;
	ssize_t len = 0;
	const char *imax_strs[] = { "3.1875mA", "6.375mA", "12.75mA",
				    "25.5mA" };
...
static DEVICE_ATTR(right_led_en, 0664, aw2015_right_led_en_show,
		   aw2015_right_led_en_store);
				...
				unsigned long *on_ms, unsigned long *off_ms)
{
	struct aw2015_led *led =
		container_of(led_cdev, struct aw2015_led, cdev);
	int rise_time_ms = 0, fall_time_ms = 0, delay_time_ms = 0,
	    slot_time_ms = 0;
				       ...
				       struct device_node *node)
{
	...
	for_each_child_of_node (node, temp) {
		...
		led->cdev.default_trigger =
			of_get_property(temp, "linux,default-trigger", NULL);
...
static bool aw2015_led_is_global(void)
{
	...
	char boot[3] = { '\0' };
	char *match =
		(char *)strnstr(saved_command_line,
				"androidboot.hwc=", strlen(saved_command_line));
	if (match) {
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (!right && aw2015_led_get_board_version() == 1 &&
	    !aw2015_led_is_global()) {
		right_led_en_gpio = of_get_named_gpio_flags(
			node, "xiaomi,right-led-en-gpio", 0, NULL);
		if (gpio_is_valid(right_led_en_gpio)) {
			if (gpio_request(right_led_en_gpio,
					 "right_led_en_gpio") == 0) {
				if (gpio_direction_output(right_led_en_gpio,
							  0)) {
